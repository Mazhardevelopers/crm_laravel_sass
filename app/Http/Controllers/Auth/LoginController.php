...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::attempt($credentials)) {
            ...
            if ($user->is_active == 1) {
                ...
                LoginDetail::create([
                    'user_id' => $user->id,
                    'ip_address' => request()->ip(), // Add parentheses to call the 'ip' method correctly
                ]);
    ...
    public function logout()
    {
        ...
        LoginDetail::where('user_id', $user->id)
            ->whereNull('logout_at') // Optional: Ensure only the active session gets updated
            ->update(['logout_at' => Carbon::now()]); // Store the current timestamp
